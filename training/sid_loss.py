...
class SID_EDMLoss:
    def __init__(self, P_mean=-1.2, P_std=1.2, sigma_data=0.5, sid_or_idmd="sid"):
        self.P_mean = P_mean
        self.P_std = P_std
        self.sigma_data = sigma_data
        self.sid_or_idmd = sid_or_idmd
    ...
    def generator_loss(
        ...
        *,
        ...
        if self.sid_or_idmd == "sid":
            loss = (y_real - y_fake) * ((y_real-y) - alpha * (y_real-y_fake)) / weight_factor
        else:
            assert self.sid_or_idmd == "idmd"
            loss = ((y_real - y)**2 - (y_fake - y)**2) / weight_factor
        return loss
